...
const taskExecuteRoutes: FastifyPluginAsync = async (fastify) => {
  ...
  fastify.post('/task/:id/complete', {
    ...
    try {
      await taskService.transition(id, 'completed', { id: req.user.userId, role: 'executor' });
    } catch (e) {
      if (e instanceof TransitionError) {
        return reply.status(409).send({ error: 'Conflict', message: e.message });
      }
    ...
    sendPushToUser(fastify.prisma, task.clientId as string, '✅ Задание выполнено', 'Оцените работу исполнителя', { task_id: id }).catch(() => {});
    return reply.send({ task_id: id, state: 'completed', completed_at: new Date() });
  });
  ...
  fastify.post('/rating', {
    ...
    const rating = await fastify.prisma.rating.create({
      data: { taskId: task_id, raterId: req.user.userId, rateeId: ratee_id, stars, comment },
    });

    // Пересчитываем рейтинг пользователя
    const stats = await fastify.prisma.rating.aggregate({
      where: { rateeId: ratee_id },
      _avg: { stars: true },
      _count: { stars: true },
    });
...
function toTaskDetail(task: Record<string, unknown>) {
  ...
  const client   = task['client']   as { name: string } | null | undefined;
  return {
    id:               task['id'],
    state:            task['state'],
